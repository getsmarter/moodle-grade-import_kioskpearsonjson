...
global $CFG, $PAGE, $DB, $OUTPUT, $COURSE;
$id = required_param('id', PARAM_INT);
// course id
$PAGE->set_url(new moodle_url('/grade/import/kioskpearsonjson/index.php', ['id' => $id]));
...
if ($data = $mform->get_data()) {
    ...
    // Setup cURL
    $authHeader = 'Authorization: '.'OAuth realm="http%3A%2F%2Ftpidev.pearsoncmg.com%2Fapi%2Ftools%2Fprofiles%2Fsearch", '.'oauth_consumer_key="'.$CFG->mylabmastering_key.'", '.'oauth_nonce="5141207439707735494", '.'oauth_signature="'.rawurlencode($sig).'", '.'oauth_signature_method="HMAC-SHA1", '.'oauth_timestamp="1345817820", '.'oauth_token="tpi", '.'oauth_version="1.0"';

    $contentTypeHeader = 'Content-Type: application/json';

    $c = new curl(['cache' => false]);
    ...
    $c->setHeader([$authHeader, $contentTypeHeader]);
