...
function import_kioskpearsonjson_grades($items, $course, &$error)
{
    ...
    if (isset($items->items)) {
        ...
        foreach ($items->items as $newItem) {
            // 1. Find old item
            $grade_item = grade_item::fetch(['idnumber' => $newItem->id, 'courseid' => $course->id]);
            ...
            foreach ($newItem->results as $newImportGrade) {
                ...
                if (!$gradeUser = $DB->get_record('user', ['id' => $newImportGrade->userId])) {
                ...
                $currentgrade = grade_grade::fetch(['itemid' => $grade_item->id, 'userid' => $gradeUser->id]);
                if (!$currentgrade) {
...
}//end import_kioskpearsonjson_grades()
